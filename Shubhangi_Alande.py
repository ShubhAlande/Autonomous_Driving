...
def train(path_to_images, csv_file):
    ...
    Trainobj = NeuralNewtworktraining(NN)    
    
    Trainobj.train_neural_network(training_X,training_Y)
...
class NeuralNewtworktraining(object):
    def __init__(self, NN):
        self.NN = NN
       
    def train_neural_network(self, training_X, training_Y):
        iterations = 3200
        alpha = 0.001
        bt1 = 0.9
        bt2 = 0.999
        epsl = 1e-08
        mvec1 = np.zeros(len(self.NN.getParams())) 
        mvec2 = np.zeros(len(self.NN.getParams())) 
        i = 0.0
        ...
        for itr in tqdm(range(iterations)):
            ...
            parameters_new = parameters - alpha*mt_hat/(np.sqrt(vt_hat)+epsl)
